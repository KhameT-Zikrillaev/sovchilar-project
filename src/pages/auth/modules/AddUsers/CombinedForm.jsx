...
const CombinedForm = ({ handleCloseModal }) => {
  ...
  const [acceptRules, setAcceptRules] = useState(user.status === "PENDING" ? false : true);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const response = await axios.put(
        `https://back.sovchilar.net/api/users-uz/${user?.id}`,
        data,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
  ...
  const inputClasses =
    "w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-transparent transition-all duration-300 text-sm";
  const selectClasses =
    "w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-transparent transition-all duration-300 appearance-none bg-white text-sm";
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                id="firstName"
                {...register("firstName", {
                  required: t("auth.CombinedForm.firstNameRequired"),
                })}
            ...
            <div>
              ...
              <input
                ...
                hdadada
                {...register("lastName", {
                  required: t("auth.CombinedForm.lastNameRequired"),
                })}
                onChange={handleNameInput}
                className={inputClasses}
            ...
            <div>
              ...
              <input
                ...
                id="telegram"
                {...register("telegram", {
                  required: t("auth.CombinedForm.telegramUsernameRequired"),
                })}
                className={inputClasses}
            ...
            <div>
              ...
              <select
                id="gender"
                {...register("gender", {
                  required: t("auth.CombinedForm.genderRequired"),
                })}
                className={selectClasses}
              >
                <option value="">{t("auth.CombinedForm.genderSelect")}</option>
                <option value="MALE">
                  {t("auth.CombinedForm.genderMale")}
                </option>
                <option value="FEMALE">
                  {t("auth.CombinedForm.genderFemale")}
                </option>
              </select>
              {errors.gender && (
                <p className="mt-1 text-sm text-rose-500">
            ...
            <div>
              ...
              <select
                id="city"
                {...register("address", {
                  required: t("auth.CombinedForm.cityRequired"),
                })}
                className={selectClasses}
              >
                <option value="">{t("auth.FormOne.selectCity.select")}</option>
                <option value="TOSHKENT">
                  {t("auth.FormOne.selectCity.Toshkent")}
                </option>
                <option value="ANDIJON">
                  {t("auth.FormOne.selectCity.Andijon")}
                </option>
                <option value="BUXORO">
                  {t("auth.FormOne.selectCity.Buxoro")}
                </option>
                <option value="FARGONA">
                  {t("auth.FormOne.selectCity.Fargona")}
                </option>
                <option value="JIZZAX">
                  {t("auth.FormOne.selectCity.Jizzax")}
                </option>
                <option value="XORAZM">
                  {t("auth.FormOne.selectCity.Xorazm")}
                </option>
                <option value="NAMANGAN">
                  {t("auth.FormOne.selectCity.Namangan")}
                </option>
                <option value="NAVOIY">
                  {t("auth.FormOne.selectCity.Navoiy")}
                </option>
                <option value="QASHQADARYO">
                  {t("auth.FormOne.selectCity.Qashqadaryo")}
                </option>
                <option value="SAMARQAND">
                  {t("auth.FormOne.selectCity.Samarqand")}
                </option>
                <option value="SIRDARYO">
                  {t("auth.FormOne.selectCity.Sirdaryo")}
                </option>
                <option value="SURXONDARYO">
                  {t("auth.FormOne.selectCity.Surxondaryo")}
                </option>
                <option value="QORAQALPOGISTON">
                  {t("auth.FormOne.selectCity.Qoraqalpogiston")}
                </option>
              </select>
              {errors.address && (
                <p className="mt-1 text-sm text-rose-500">
            ...
            <div>
              ...
              <select
                ...
                {...register("qualification", {
                  required: t("auth.CombinedForm.educationRequired"),
                })}
                className={selectClasses}
              >
                <option value="">{t("auth.FormTwo.selectEducation")}</option>
                <option value="middle">
                  {t("auth.FormTwo.qualification.middle")}
                </option>
                <option value="specialized">
                  {t("auth.FormTwo.qualification.specialized")}
                </option>
                <option value="higher">
                  {t("auth.FormTwo.qualification.higher")}
                </option>
                <option value="master">
                  {t("auth.FormTwo.qualification.master")}
                </option>
                <option value="doctorate">
                  {t("auth.FormTwo.qualification.doctorate")}
                </option>
              </select>
            ...
            <div>
              ...
              <select
                ...
                {...register("maritalStatus", {
                  required: t("auth.CombinedForm.maritalStatusRequired"),
                })}
                className={selectClasses}
              >
                <option value="">
                  {t("auth.FormTwo.maritalStatus.default")}
                </option>
                {watch("gender") === "MALE" ? (
                  <>
                    <option value="SINGLE">
                      {t("auth.FormTwo.maritalStatus.male.single")}
                    </option>
                    <option value="DIVORCED">
                      {t("auth.FormTwo.maritalStatus.male.divorced")}
                    </option>
                    <option value="MARRIED_SECOND">
                      {t("auth.FormTwo.maritalStatus.male.married_second")}
                    </option>
                  </>
                ) : watch("gender") === "FEMALE" ? (
                  <>
                    <option value="SINGLE">
                      {t("auth.FormTwo.maritalStatus.female.single")}
                    </option>
                    <option value="DIVORCED">
                      {t("auth.FormTwo.maritalStatus.female.divorced")}
                    </option>
                  </>
                ) : null}
              </select>
            ...
            <div>
              ...
              <input
                ...
                id="jobTitle"
                {...register("jobTitle", {
                  required: t("auth.CombinedForm.occupationRequired"),
                })}
                className={inputClasses}
            ...
            <div>
              ...
              <select
                ...
                {...register("nationality", {
                  required: t("auth.CombinedForm.ethnicityRequired"),
                })}
                className={selectClasses}
              >
                <option value="">
                  {t("auth.FormTwo.selectNationality.select")}
                </option>
                <option value="Uzbek">
                  {t("auth.FormTwo.selectNationality.Uzbek")}
                </option>
                <option value="Russian">
                  {t("auth.FormTwo.selectNationality.Russian")}
                </option>
                <option value="Kazakh">
                  {t("auth.FormTwo.selectNationality.Kazakh")}
                </option>
                <option value="Kyrgyz">
                  {t("auth.FormTwo.selectNationality.Kyrgyz")}
                </option>
                <option value="Tajik">
                  {t("auth.FormTwo.selectNationality.Tajik")}
                </option>
                <option value="Turkmen">
                  {t("auth.FormTwo.selectNationality.Turkmen")}
                </option>
                <option value="Tatar">
                  {t("auth.FormTwo.selectNationality.Tatar")}
                </option>
                <option value="Karakalpak">
                  {t("auth.FormTwo.selectNationality.Karakalpak")}
                </option>
                <option value="Other">
                  {t("auth.FormTwo.selectNationality.Other")}
                </option>
              </select>
              {errors.nationality && (
                <p className="mt-1 text-sm text-rose-500">
          ...
          <div>
            ...
            <textarea
              ...
              {...register("description", {
                ...
                minLength: {
                  value: 10,
                  message: t("auth.CombinedForm.requirementsMinLength"),
                },
              })}
              rows="4"
              className={inputClasses}
              ...
              <div className="prose max-w-none">
                ...
                <div className="mt-4">
                  ...
                  <ul className="list-disc pl-5 space-y-2">
                    <li>
                      <strong>{t("rules.administration")}</strong> –{" "}
                      {t("rules.siteStaff")}
                    </li>
                    <li>
                      <strong>{t("rules.services")}</strong> –{" "}
                      {t("rules.softwareCollection")}
                    </li>
                    <li>
                      <strong>{t("rules.user")}</strong> –{" "}
                      {t("rules.personOver18")}
                    </li>
                    <li>
                      <strong>{t("rules.login")}</strong> –{" "}
                      {t("rules.chosenLogin")}
                    </li>
                    <li>
                      <strong>{t("rules.password")}</strong> –{" "}
                      {t("rules.chosenPassword")}
                    </li>
                  </ul>
                </div>
                <div className="mt-6">
                  <h3 className="font-semibold mb-2">
                    {t("rules.generalRules")}
                  </h3>
                  <p>{t("rules.rulesExplanation")}</p>
                </div>
                <div className="mt-6">
                  <h3 className="font-semibold mb-2">
                    {t("rules.confidentialInfo")}
                  </h3>
